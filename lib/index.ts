...
class MPDClient extends EventEmitter {
	...
	static connect(config?: MPDConfig): Promise<MPDClient> {
		...
		return finalizeClientConnection(new MPDClient(configToUse), socket);
	}
...
const finalizeClientConnection = (
	client: MPDClient,
	socket: net.Socket,
): Promise<MPDClient> =>
	new Promise((resolve, reject) => {
		...
		const finalize = () => {
			...
			Object.defineProperty(client, "PROTOCOL_VERSION", {
		...
		const onData = (data: string) => {
			...
			if (!MPD_SENTINEL.test(data)) {
				...
				reject(
					new MPDError(
						"Unexpected MPD service response",
						"INVALIDMPDSERVICE",
						`got: '${data}'`,
					),
				);
