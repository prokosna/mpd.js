 ...
 *     ]
 */
export const parseList = <T = Record<string, any>>(
	msg: string,
	delimiters?: string | string[] | Record<string, boolean>,
): T[] =>
	msg.split("\n").reduce(
		(memo: ParseListMemo<T>, line: string) => {
			if (ignoreLine(line)) {
				return memo;
			}
...
parseList.by = <T = Record<string, any>>(
	...delimiters: (string | string[] | Record<string, boolean>)[]
) => {
 ...
 */
export const parseObject = <T = Record<string, any>>(
	msg: string,
): T | undefined => parseList<T>(msg)[0];
 ...
 *     ablum: [ { ablum: 'cactusalbum', title: [ { title: 'bull' } ] } ] } ]
 */
export const parseNestedList = <T = Record<string, any>>(msg: string): T[] =>
	msg.split("\n").reduce(
		(memo: ParseNestedListMemo<T>, line: string) => {
			if (ignoreLine(line)) {
				return memo;
			}
 ...
 */
export const parseListAndAccumulate =
	(path: string[]) =>
	<T = Record<string, any>>(msg: string): T[] =>
		msg.split("\n").reduce(
			(memo: ParseListAndAccumulateMemo<T>, line: string) => {
				if (ignoreLine(line)) {
					return memo;
				}
	...
	toShortUnit(num: number, digits?: number): ShortUnitResult {
		...
		return {
			value: parsers.parseFloat(
				(num / si[ii].value).toFixed(digitsToUse).replace(rx, "$1"),
			),
			unit: si[ii].symbol,
		};
	},
